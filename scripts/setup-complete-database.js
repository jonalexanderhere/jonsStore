...
async function setupCompleteDatabase() {
  ...
  try {
    ...
    // 2. Read and execute complete SQL script
    console.log('2️⃣ Executing complete database setup script...')
    const sqlScript = fs.readFileSync(path.join(__dirname, '../supabase-complete-setup.sql'), 'utf8')
    ...
    console.log('\n6️⃣ Testing user registration flow...')
    ...
    if (testUserError) {
      console.error('❌ Test user creation failed:', testUserError.message)
    } else {
      ...
      const { data: testProfile } = await supabase
        .from('users')
        .select('*')
        .eq('id', testUserData.user.id)
        .single()

      if (testProfile) {
        console.log('✅ User profile created automatically by trigger!')
        console.log('👤 Profile:', testProfile.full_name, `(${testProfile.role})`)
      } else {
        console.log('⚠️  User profile not created automatically')
      }
    }
    ...
    console.log('✅ Complete table structure created')
