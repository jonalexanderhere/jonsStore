...
async function setupWithCLI() {
  try {
    console.log('🚀 Setting up Supabase database with CLI...')
    console.log('🔗 Supabase URL:', supabaseUrl)

    // Step 1: Create a temporary SQL file
    console.log('\n📝 Creating SQL file...')
    
    const sqlContent = `-- Create categories table
...
CREATE POLICY "Enable insert for authenticated users only" ON order_items FOR INSERT WITH CHECK (auth.role() = 'authenticated');`
    ...
    // Step 2: Try to execute SQL using Supabase CLI
    console.log('\n🚀 Executing SQL with Supabase CLI...')
    
    try {
      // Try to link to project first
      console.log('🔗 Linking to Supabase project...')
      execSync(`npx supabase link --project-ref ${supabaseUrl.split('//')[1].split('.')[0]}`, { stdio: 'inherit' })
      
      // Execute SQL
      console.log('📝 Executing SQL...')
      execSync(`npx supabase db reset --db-url ${supabaseUrl}`, { stdio: 'inherit' })
      
      console.log('✅ Database setup completed with CLI!')
      
    } catch (cliError) {
      console.log('❌ CLI method failed, trying manual approach...')
      console.log('📋 Manual setup required:')
      console.log('1. Go to Supabase Dashboard → SQL Editor')
      console.log('2. Copy and run the content of supabase-setup.sql')
      console.log('3. Or copy the content below:')
      console.log('\n' + '='.repeat(50))
    ...
    console.log('\n📋 Next steps:')
    console.log('1. If manual setup was required, complete it in Supabase Dashboard')
