-- =====================================================
-- PRODUCTION-READY REALTIME E-COMMERCE DATABASE
-- Modern E-commerce Platform with Full Real-time Capabilities
-- =====================================================
...
-- =====================================================
-- CREATE TABLES WITH REALTIME ENABLED
-- =====================================================
...
-- Analytics table for real-time tracking
...
BEGIN
    IF OLD.status != NEW.status THEN
        ...
        VALUES (
            ...
            'Pesanan ' || NEW.order_number || ' status: ' || NEW.status,
            'info',
...
CREATE TRIGGER order_status_notification AFTER UPDATE ON orders FOR EACH ROW EXECUTE FUNCTION create_order_notification();

-- Function to track product views
CREATE OR REPLACE FUNCTION track_product_view()
RETURNS TRIGGER AS $$
BEGIN
    INSERT INTO analytics (event_type, product_id, data)
    VALUES ('product_view', NEW.id, jsonb_build_object('product_name', NEW.name, 'price', NEW.price));
    RETURN NEW;
END;
$$ language 'plpgsql';
...
CREATE POLICY "Admin can view analytics" ON analytics FOR SELECT USING (
    EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role = 'admin')
);
...
BEGIN
    ...
    SELECT 
        ...
        (SELECT COALESCE(SUM(total_amount), 0) FROM orders WHERE DATE(created_at) = CURRENT_DATE AND payment_status = 'paid');
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
-- 2. Real-time capabilities enabled for all tables
-- 3. Row Level Security (RLS) policies for data protection
-- 4. Indexes for optimal performance
-- 5. Triggers for automatic timestamp updates and notifications
-- 6. Sample data for testing
-- 7. Views for common queries
-- 8. Utility functions for cart, recommendations, and analytics
-- 9. Proper permissions for different user roles
-- 10. Real-time notifications and analytics tracking
...
-- 6. Enable real-time subscriptions in your frontend
